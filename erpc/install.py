...
Inspired by py-geth's install module.
"""
...
# Map platform.system() + platform.machine() to release artifact names
PLATFORM_MAP = {
...
def install_erpc(
    ...
    logger.info(f"Downloading eRPC {version} from {url}")
    urllib.request.urlretrieve(url, str(dest))

    # Make executable
    dest.chmod(dest.stat().st_mode | stat.S_IEXEC | stat.S_IXGRP | stat.S_IXOTH)

    logger.info(f"Installed eRPC {version} to {dest}")
    return dest
